...
<script type="text/javascript">
	...
	function sendMsg() {
		...
		var url = "http://localhost:8888/LogisticsBusiness/cxf/WaybillServiceImpl"; //webService访问链接
		var requestBody = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:q0=\"http://impl.service.lb.briup.com/\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">"
	...
	function _back() {
		if (xmlHttpRequest.readyState == 4) { //提交完成后
			if (xmlHttpRequest.status == 200) { //调用成功
				var resXml = xmlHttpRequest.responseXML;

				var ret = resXml.getElementsByTagName("return")[0]; //获取return节点信息
				...
				if (ret != null) {
					...
					var lcno = ret.getElementsByTagName("lcno")[0].firstChild.nodeValue;
			...
			<div id="middleMenubar">
				<div id="innerMenubar">
					<div id="navMenubar">
						<ul>
							<li id="view"><a href="#"
								onclick="javascript:sendMsg();this.blur();">查看</a></li>
						</ul>
					</div>
				</div>
			</div>
		...
		<div>
			<table class="commonTable" cellspacing="1">
				<tr>
					...
					<td class="tableContent"><select name="waybillId" id="id"
						onchange="setWaybillId(this.options[this.selectedIndex].text);">
							<option value="">--请选择--</option>
							<c:forEach items="${waybillList}" var="w">
				...
				<tr>
					...
					<td class="columnTitle_mustbe">成交方式：</td>
