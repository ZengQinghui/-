...
<script type="text/javascript">
	...
	function addTRRecord(objId, id, productNo, cnumber, grossWeight, netWeight,
		...
		oTD.innerHTML = "<input type=\"text\" name=\"mr_cnumber\" maxLength=\"10\" value=\""
				+ cnumber + "\" onBlur=\"setTRUpdateFlagTrue(this);\" size=\"15\">";
		oTD = oTR.insertCell(5);
		oTD.innerHTML = "<input type=\"text\" name=\"mr_grossWeight\" maxLength=\"10\" value=\""
				+ grossWeight
				+ "\" onBlur=\"setTRUpdateFlagTrue(this);\" size=\"15\">";
		...
		oTD.innerHTML = "<input type=\"text\" name=\"mr_exPrice\" maxLength=\"10\" value=\""
				+ exPrice + "\" onBlur=\"setTRUpdateFlagTrue(this);\" size=\"15\">";
		oTD = oTR.insertCell(11);
		oTD.innerHTML = "<input type=\"text\" name=\"mr_tax\" maxLength=\"10\" value=\""
				+ tax + "\" onBlur=\"setTRUpdateFlagTrue(this);\" size=\"15\">";
			...
			<div id="middleMenubar">
				<div id="innerMenubar">
					<div id="navMenubar">
						<ul>
							<li id="save"><a href="#"
								onclick="formSubmit('${ctx}/freight/export/doUpdate.action','_self');">确定</a></li>
		...
		<div class="textbox" id="centerTextbox">
			...
			<div class="textbox-header">
				<div class="textbox-inner-header">
					<div class="textbox-title">修改出口报运信息</div>
				</div>
			</div>
			<div>
				<div>
					<table class="commonTable" cellspacing="1">
						<tr>
							...
							<td class="tableContent"><input type="text"
								...
								value="<fmt:formatDate value="${waybill.inputDate}" pattern="yyyy-MM-dd"/>"
						...
						<tr>
							...
							<td class="tableContent"><input type="text" name="consignee"
								value="${waybill.consignee}" /></td>
						</tr>
						<tr>
							<td class="columnTitle_mustbe">装运港：</td>
							<td class="tableContent"><input type="text"
								name="shipmentPort" value="${waybill.shipmentPort}" /></td>
							<td class="columnTitle_mustbe">目的港：</td>
							<td class="tableContent"><input type="text"
								name="destinationPort" value="${waybill.destinationPort}" /></td>
						</tr>
						<tr>
							<td class="columnTitle_mustbe">运输方式：</td>
							<td class="tableContentAuto"><input type="radio"
								name="transportMode" value="SEA" class="input"
								<c:if test="${waybill.transportMode=='SEA'}">checked</c:if>>SEA
								<input type="radio" name="transportMode" value="AIR"
								class="input"
								<c:if test="${waybill.transportMode=='AIR'}">checked</c:if>>AIR
							</td>
							<td class="columnTitle_mustbe">价格条件：</td>
							<td class="tableContentAuto"><input type="radio"
								name="priceCondition" value="FOB" class="input"
								<c:if test="${waybill.priceCondition=='FOB'}">checked</c:if>>FOB
								<input type="radio" name="priceCondition" value="CIF"
								class="input"
								<c:if test="${waybill.priceCondition=='CIF'}">checked</c:if>>CIF
							</td>
						</tr>
						<tr>
							<td class="columnTitle_mustbe">唛头：</td>
							<td class="tableContent"><textarea name="marks"
									style="height: 120px;">${waybill.marks}</textarea></td>
							<td class="columnTitle_mustbe">备注：</td>
							<td class="tableContent"><textarea name="remark"
									style="height: 120px;">${waybill.remark}</textarea></td>
						</tr>
					</table>
				</div>
			</div>
