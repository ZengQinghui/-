					...
					<table class="commonTable" cellspacing="1">
						<tr>
							...
							<td class="tableContent"><input type="text"
								name="customerName" value="${contract.customerName}" /></td>
							<td class="columnTitle_mustbe">收购方：</td>
							<td class="tableContent"><input type="text" name="acquiringFirm"
								value="${contract.acquiringFirm}" /></td>
						</tr>
						<tr>
							<td class="columnTitle_mustbe">合同号：</td>
							<td class="tableContent"><input type="text"
								name="contractNo" value="${contract.contractNo}" /></td>
							<td class="columnTitle_mustbe">打印版式：</td>
							<td class="tableContentAuto"><input type="radio"
								name="printStyle" value="2" class="input"
								<c:if test="${contract.printStyle=='2'}">checked</c:if>>两款 <input
								type="radio" name="printStyle" value="1" class="input"
								<c:if test="${contract.printStyle=='1'}">checked</c:if>>一款</td>
						</tr>
						<tr>
							<td class="columnTitle_mustbe">签单日期：</td>
							<td class="tableContent"><input type="text"
								style="width: 90px;" name="signingDate"
							...
							<td class="tableContentAuto"><input type="radio"
								...
								<c:if test="${contract.importNum==3}">checked</c:if>>★★★ <input
								type="radio" name="importNum" value="2" class="input"
								<c:if test="${contract.importNum==2}">checked</c:if>>★★ <input
								type="radio" name="importNum" value="1" class="input"
								<c:if test="${contract.importNum==1}">checked</c:if>>★</td>
						</tr>
						<tr>
							...
							<td class="tableContent"><input type="text"
								style="width: 90px;" name="shipingDate"
								value="<fmt:formatDate value="${contract.shipingDate}" pattern="yyyy-MM-dd"/>"
								onclick="WdatePicker({el:this,isShowOthers:true,dateFmt:'yyyy-MM-dd'});" />
							</td>
						</tr>
						<tr>
