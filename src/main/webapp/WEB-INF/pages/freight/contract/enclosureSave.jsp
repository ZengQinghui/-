...
<body>
	<form method="post" enctype="multipart/form-data">
		<input type="hidden" name="contractProductId"
			...
			<div id="middleMenubar">
				<div id="innerMenubar">
					<div id="navMenubar">
						<ul>
							<li id="save"><a href="#"
								onclick="formSubmit('doEnclosureSave.action','_self');">确定</a></li>
				...
				<div>
					<table class="commonTable" cellspacing="1">
						...
						<tr>
							...
							<td class="tableContent"><select name="ctype">
									...
									<c:forEach items="${allTypes}" var="type">
										<option value="${type.orderNo}">${type.name}</option>
									</c:forEach>
							</select></td>
						...
						<tr>
							...
							<td class="tableContent"><input type="text" name="quantity" /></td>
					...
					<div class="eXtremeTable">
						<table id="ec_table" class="tableRegion" width="98%">
							...
							<tbody class="tableBody">
								...
								<c:forEach items="${allEnclosures}" var="enclosure" varStatus="status">
									<tr class="odd" onmouseover="this.className='highlight'"
										onmouseout="this.className='odd'">
										<td><input type="checkbox" name="id" value="${enclosure.id}" /></td>
										<td>${status.index+1}</td>
										<td>${enclosure.factoryName}</td>
										<td>${enclosure.productNo}</td>
										<td>${enclosure.quantity}</td>
										<td>${enclosure.packingUnit}</td>
										<td>${enclosure.price}</td>
										<td>${enclosure.amount}</td>
										<td><a href="toEnclosureUpdate.action?id=${enclosure.id}">[修改]</a> <a
											href="doEnclosureDelete.action?id=${enclosure.id}&contractProductId=${enclosure.contractProductId}">[删除]</a>
										</td>
									</tr>
								</c:forEach>
