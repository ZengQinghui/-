...
<link rel="stylesheet" rev="stylesheet" type="text/css"
	href="${ctx}/skin/default/css/title.css" media="all" />
...
<script language="javascript">
	function CustomTitle() {
		...
		Me.onmouseover = function() {
			document.getElementById('memo').style.background = 'url(${ctx}/skin/default/images/title/memo2.gif) no-repeat'
		};
		Me.onmouseout = function() {
			document.getElementById('memo').style.background = 'url(${ctx}/skin/default/images/title/memo.gif) no-repeat'
		};

		Loin.onmouseover = function() {
			document.getElementById('small_login').style.background = 'url(${ctx}/skin/default/images/title/small_login2.gif) no-repeat'
		};
		Loin.onmouseout = function() {
			document.getElementById('small_login').style.background = 'url(${ctx}/skin/default/images/title/small_login.gif) no-repeat'
		};

		Lout.onmouseover = function() {
			document.getElementById('small_login_out').style.background = 'url(${ctx}/skin/default/images/title/login_out2.gif) no-repeat left -55px;'
		};
		Lout.onmouseout = function() {
			document.getElementById('small_login_out').style.background = 'url(${ctx}/skin/default/images/title/login_out.gif) no-repeat'
		};
	}
	...
	function ShowFrameMain(passValue) {
		...
		if (passValue) {
			...
			form1.innerHTML = form1.innerHTML
					+ '<input type="hidden"  name="passContent" value="' +passValue[1]+ '"/>';
		}
		form1.submit();
	...
	function checkDirectionKey() {
		...
		if (!(0 - currLeft) <= maxOffset || currLeft < 0) {
			...
			$("#leftKey").show("slow");
			if (!isShow) {
				$("#prompt_div").show("slow", function() {
					window.setTimeout(function() {
						$("#prompt_div").hide("slow")
					}, 10000);
					isShow = true;
				});
			}
	...
	function logout() {
		return formSubmit("${ctx}/logout.action", "_top");
	}
</script>
...
<body onSelectStart="return true">
	...
	<div id="userInfo" style="z-index: 999;" onclick="HideLoginDiv()"
		title="点击关闭">
		<img src="${ctx}/skin/default/images/title/avataronline.gif"
			border="0" style="margin-top: -1px;" /> 您好：<strong>${user.userName}</strong>&nbsp;&nbsp;|
		您所属部门：<strong title="${user.deptName}">${user.deptName}</strong>&nbsp;&nbsp;
		<img src="${ctx}/skin/default/images/title/close.gif" border="0" />
	</div>

	<a id="memos" style="cursor: pointer;" href="${ctx}/home.action"
		target="_top" title="点击切换到系统首页"><div id="memo" class="memo"
			title="点击切换到系统首页"></div></a>
	<a id="logins" style="cursor: pointer;" onclick="doLoginDiv();"
		title="点击显示您的登录信息"><div id="small_login" class="small_login"
			title="点击显示您的登录信息"></div></a>
	<a id="logout" style="cursor: pointer;" onclick="logout();"
		target="_top" title="点击退出系统"><div id="small_login_out"
			class="small_login_out" title="点击退出系统"></div></a>

	<div class="headerBg">
		<div class="top_logo">
			<div class="navMenu" style="float: left; text-align: left;">
				<div class="titleDate" style="float: left;">
					<fmt:formatDate value="${now}" pattern="yyyy年M月d日 E" />
				</div>
				<div style="height: 29px;">
					<span id="leftKey" onmouseover="periodOffset(this, 'left')"><img
						src="${ctx}/skin/default/images/title/left_arrow.png" /></span>
					<div class="mavMeau_top"></div>
					<div id="mask">
						<div id="menuContent">
							<span id="topmenu"
								onclick="top.location.href='home.action';linkHighlightMenu(this);">系统首页</span><span
								id="tm_separator"></span><span id="topmenu"
								onclick="top.leftFrame.location.href='freightLeft.action';top.main.location.href='freightMain.action';linkHighlightMenu(this);">货运管理</span><span
								id="tm_separator"></span><span id="topmenu"
								...
								id="tm_separator"></span><span id="topmenu"
								onclick="top.leftFrame.location.href='baseinfoLeft.action';top.main.location.href='baseinfoMain.action';linkHighlightMenu(this);">基础信息</span><span
								id="tm_separator"></span><span id="topmenu"
					...
					<span id="rightKey" onmouseover="periodOffset(this, 'right')"><img
						src="${ctx}/skin/default/images/title/right_arrow.png" /></span>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="prompt_div">
		<img src="${ctx}/skin/default/images/title/prompt.png" /><span
			style="position: absolute; top: 2px; left: 35px; z-index: 99999; width: 100%; color: #FFFFFF; text-align: left;">鼠标指向箭头位置<br />可显示更多菜单项
		</span>
	</div>
